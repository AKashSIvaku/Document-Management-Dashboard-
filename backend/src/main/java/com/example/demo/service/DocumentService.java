...
public class DocumentService {
    ...
    public void processBulkFilesAsync(List<MultipartFile> files, String batchId) {
        int successCount = 0;
        int failCount = 0;
    ...
    /**
     * Synchronously caches files in memory to avoid Tomcat cleaning up temp multipart files
     * before the async background processing completes.
     */
    public List<MultipartFile> cacheFiles(MultipartFile[] files) {
        List<MultipartFile> cachedList = new ArrayList<>();
        for (MultipartFile file : files) {
            try {
                cachedList.add(new ByteArrayMultipartFile(
                        file.getName(),
                ...
                log.error("Failed to cache file {}", file.getOriginalFilename(), e);
            }
        }
        return cachedList;
    }
    ...
    /**
     * Custom implementation of MultipartFile that wraps a byte array in memory.
     */
